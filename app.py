        ...
        <tr>
            ...
            <td>
                <div style="display:flex; align-items:center; justify-content:space-between; gap:10px;">
                    {form_assegnazione}
                    <a href="/elimina-voce?id={s.id}&data={str_data_lavoro}" class="btn-xs" style="text-decoration:none;">Elimina</a>
                </div>
            </td>
        </tr>
        """
...
def elimina_voce(request: Request, id: int, data: str, db: Session = Depends(get_db)):
    if not utente_autenticato(request):
        return RedirectResponse(url="/login", status_code=303)
...
def salva_avviso(request: Request, testo_avviso: str = Form(...), data_corrente: str = Form(...), db: Session = Depends(get_db)):
    if not utente_autenticato(request):
        return RedirectResponse(url="/login", status_code=303)
...
@app.get("/esporta-log")
def esporta_log(request: Request, tipo: str, data: str, db: Session = Depends(get_db)):
    if not utente_autenticato(request):
        return RedirectResponse(url="/login", status_code=303)
...
async def import_motore(request: Request, file: UploadFile = File(...), db: Session = Depends(get_db)):
    if not utente_autenticato(request):
        return RedirectResponse(url="/login", status_code=303)
...
def assegna_manuale(request: Request, sostituzione_id: int = Form(...), sostituto_id: str = Form(...), data_corrente: str = Form(...), db: Session = Depends(get_db)):
    if not utente_autenticato(request):
        return RedirectResponse(url="/login", status_code=303)
...
def reset_totale(request: Request, db: Session = Depends(get_db)):
    if not utente_autenticato(request):
        return RedirectResponse(url="/login", status_code=303)
...
def totem_corridoio(db: Session = Depends(get_db)):
    ...
    <body>
        ...
        <script>
            ...
            function mostraPaginaCarosello() {{
